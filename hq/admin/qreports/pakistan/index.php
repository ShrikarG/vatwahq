...
require '../../connect.php';
                ...
                if($result = $db->query("SELECT * FROM qr_staff where vacc='Pakistan vACC'"))
                {
		            if($result->num_rows)
			    {	
            ...
            $events = array();
                ...
                if($result = $db->query("SELECT * FROM events where vacc='Pakistan vACC'"))
                {
		            if($result->num_rows)
			    {	
				while($row  = $result->fetch_object())
            ...
            $atc = array();
                ...
                if($result = $db->query("SELECT * FROM atc_roster where vacc='Pakistan vACC' AND status='Resident'"))
                {
		            if($result->num_rows)
			    {	
				while($row  = $result->fetch_object())
...
<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h3><rw1>
        Quarterly Report for Pakistan vACC
      </h3></rw12>
    </section>
		...
		<tbody>
            ...
            <input type = "hidden" name = "vacc" value = "Pakistan vACC">
            </div>
            </div>
            <br>
            <center>
...
?>      
		...
		<tbody>
            ...
            <input class = "form-control" name="vacc" value="Pakistan vACC" type="hidden" />
            </div>
            <br>
            <center>
            <input class="btn btn-info" name="save" type="submit" />
 ...
 <div class="card">
     ...
     <form name = "staff" action="admin/qreports/insert/q_status.php" method="POST">
            ...
            <input class = "form-control" name="vacc" type="hidden" value="Pakistan vACC" />
